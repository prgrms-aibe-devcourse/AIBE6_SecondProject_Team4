...
export default function SentLessonRequestsPage() {
    const { user } = useAuth()
    ...
    useEffect(() => {
        ...
        const loadRequests = async () => {
            try {
                ...
                const { data, error } = await client.GET('/api/members/me/lesson-requests')

                if (error || !data) {
                    setErrorMessage('보낸 레슨 요청을 불러오지 못했습니다.')
                    return
                }
    ...
    if (user?.role === 'TRAINER') {
        return (
            <main className="flex-1 bg-background pt-16 md:pt-20">
                <PageMessage icon="lock" title="일반 회원만 보낸 요청을 확인할 수 있습니다." />
            </main>
        )
    }
        ...
        <main className="flex-1 bg-background pt-16 md:pt-20">
            <section className="mx-auto w-full max-w-screen-xl px-margin-mobile py-xl md:px-margin-desktop">
                ...
                <p className="mt-xs text-body-md text-on-surface-variant">
                    트레이너에게 보낸 요청과 처리 상태를 확인할 수 있습니다.
                </p>
                ...
                {isLoading ? (
                    ...
                    <PageMessage icon="outgoing_mail" title="해당하는 레슨 요청이 없습니다." />
                ) : (
                    <div className="mt-md space-y-sm">
                        {filteredRequests.map((request) => (
                            <SentRequestItem key={request.lessonRequestId} request={request} />
                        ))}
                    </div>
                )}
            </section>
...
function SentRequestItem({ request }: { request: LessonRequest }) {
            ...
            <div className="flex min-w-0 flex-1 items-center gap-sm">
                <div className="h-14 w-14 shrink-0 overflow-hidden rounded-full bg-surface-container">
                    {request.trainerProfileImage ? (
                        <img
                            src={getImageUrl(request.trainerProfileImage)}
                            alt={`${request.trainerName ?? '트레이너'} 프로필`}
